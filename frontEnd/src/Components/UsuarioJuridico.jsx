          ...
          {loadingDocs ? (
            ...
            return (
              <div className="mt-4">
                ...
                {(() => {
                  const docsTecnico = documentosDoc.filter(f => {
                    const rol = (f.rol_nombre || "").toLowerCase();
                    const cat = (f.categoria || "").toLowerCase();
                    return rol.includes("tecnico") || rol.includes("técnico") || cat === "informe_tecnico";
                  });

                  if (docsTecnico.length > 0) {
                    return (
                      <div className="mt-4 pt-4 border-top">
                        <div className="d-flex align-items-center mb-3">
                          <i className="bi bi-file-earmark-medical fs-5 me-2 text-success"></i>
                          <h6 className="mb-0 fw-bold" style={{ fontSize: '1.1rem', color: '#333' }}>Área Técnica</h6>
                        </div>

                        <div className="listado-archivos-tecnico">
                          {docsTecnico.map((f, i) => (
                            <div key={i} className="doc-subido-item-nuevo mb-2 p-2 border rounded bg-white shadow-sm d-flex justify-content-between align-items-center">
                              <div className="d-flex align-items-center overflow-hidden">
                                <i className="bi bi-file-earmark-pdf text-success me-2 fs-5"></i>
                                <div className="text-truncate">
                                  <div className="fw-semibold text-truncate" style={{ fontSize: '0.9rem' }}>{f.nombre_archivo}</div>
                                  <div className="text-muted" style={{ fontSize: '0.75rem' }}>
                                    Subido el {f.fecha_subida ? new Date(f.fecha_subida).toLocaleDateString('es-AR') : '—'} 
                ...
                {/* SECCIÓN: ÁREA JURÍDICA */}
                {(() => {
                  const docsJuridico = documentosDoc.filter(f => {
                    const rol = (f.rol_nombre || "").toLowerCase();
                    const cat = (f.categoria || "").toLowerCase();
                    return rol.includes("juridico") || rol.includes("jurídico") || cat === "informe_juridico";
                  });

                  if (docsJuridico.length > 0) {
                    return (
                      <div className="mt-4 pt-4 border-top">
                        <div className="d-flex align-items-center mb-3">
                          <i className="bi bi-file-earmark-gavel fs-5 me-2 text-info"></i>
                          <h6 className="mb-0 fw-bold" style={{ fontSize: '1.1rem', color: '#333' }}>Área Jurídica</h6>
                        </div>

                        <div className="listado-archivos-juridico">
                          {docsJuridico.map((f, i) => (
                            <div key={i} className="doc-subido-item-nuevo mb-2 p-2 border rounded bg-white shadow-sm d-flex justify-content-between align-items-center">
                              <div className="d-flex align-items-center overflow-hidden">
                                <i className="bi bi-file-earmark-pdf text-info me-2 fs-5"></i>
                                <div className="text-truncate">
                                  <div className="fw-semibold text-truncate" style={{ fontSize: '0.9rem' }}>{f.nombre_archivo}</div>
                                  <div className="text-muted" style={{ fontSize: '0.75rem' }}>
                                    Subido el {f.fecha_subida ? new Date(f.fecha_subida).toLocaleDateString('es-AR') : '—'} 
          ...
          {expedienteVer && (
            <>
              ...
              <h5 className="section-subtitle-premium mb-3 mt-4">
                <i className="bi bi-file-earmark-gavel me-2"></i>Informe Jurídico
              </h5>
              ...
              <h5 className="section-subtitle-premium mb-3">
                <i className="bi bi-person-badge me-2"></i>Dirección destino
              </h5>
              <Form.Group controlId="formDestinatarioPase" className="mb-4">
                ...
                >
                  <option value="">— Seleccioná un director —</option>
                  {usuariosPase.map(u => (
                ...
                {usuariosPase.length === 0 && (
                  <Form.Text className="text-danger mt-1 d-block">
                    ...
                    No hay usuarios directores disponibles.
                  </Form.Text>
                )}
              </Form.Group>
